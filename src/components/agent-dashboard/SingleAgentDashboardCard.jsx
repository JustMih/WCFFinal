...
export default function SingleAgentDashboardCard() {
  ...
  // ✅ voice mail stats (derived from DB)
  const [voiceMailStats, setVoiceMailStats] = useState({
    total: 0,
    played: 0,
    notPlayed: 0,
  });
  ...
  useEffect(() => {
    ...
    /* ===============================
       1️⃣ Agent call statistics
    =============================== */
    fetch(`${baseURL}/calls/agent-calls-today/${agentExtension}`)
      .then((res) => {
        if (!res.ok) throw new Error("Failed to fetch agent stats");
        return res.json();
      })
    ...
    fetch(`${baseURL}/voice-notes`, {
      ...
      .catch(() =>
        setVoiceMailStats({ total: 0, played: 0, notPlayed: 0 })
      );
  }, []);
  ...
  /* ===============================
     Generic Call Cards
  =============================== */
  const renderCard = (type, Icon, title, dataKey, colorClass) => (
    <div className={`single-agent-card modern-agent-card ${type}`}>
      <div className="single-agent-head">
        <Icon fontSize={22} />
        <span>{title}</span>
      ...
      <div className="single-agent-stats-row">
        <div className="stat-block">
          <span className="stat-label">Total</span>
          <span className={`stat-value ${colorClass}-color`}>
            {safe(stats?.[dataKey], "total")}
          </span>
        </div>
        <div className="stat-block">
          <span className="stat-label">Answered</span>
          <span className="stat-value answered-color">
            {safe(stats?.[dataKey], "answered")}
          </span>
        </div>
        <div className="stat-block">
          <span className="stat-label">Dropped</span>
          <span className="stat-value dropped-color">
            {safe(stats?.[dataKey], "dropped")}
          </span>
        </div>
        <div className="stat-block">
          <span className="stat-label">Lost</span>
          <span className="stat-value lost-color">
            {safe(stats?.[dataKey], "lost")}
          </span>
        </div>
      </div>
    </div>
    ...
    <div className="single-agent-card modern-agent-card social-messages">
      ...
      <div className="single-agent-stats-row icon-badge-row">
        <div className="icon-badge">
          ...
          <span className="badge whatsapp-color">
            {safe(stats?.whatsapp, "total")}
          </span>
        </div>
        <div className="icon-badge">
          <FaInstagram fontSize={24} />
          <span className="badge instagram-color">
            {safe(stats?.instagram, "total")}
          </span>
        </div>
        <div className="icon-badge">
          <FaXTwitter fontSize={24} />
          <span className="badge twitter-color">
            {safe(stats?.twitter, "total")}
          </span>
        </div>
        <div className="icon-badge">
          <TbMail fontSize={24} />
          <span className="badge email-color">
            {safe(stats?.email, "total")}
          </span>
        </div>
      </div>
    </div>
  ...
  const renderVoicemailCard = () => (
    <div className="single-agent-card modern-agent-card voicemail">
      ...
      <div className="single-agent-stats-row cols-3">
        <div className="stat-block">
          <span className="stat-label">Total</span>
          <span className="stat-value voicemail-color">
            {voiceMailStats.total}
          </span>
        </div>
        <div className="stat-block">
          <span className="stat-label">Played</span>
          <span className="stat-value answered-color">
            {voiceMailStats.played}
          </span>
        </div>
        <div className="stat-block">
          <span className="stat-label">Not Played</span>
          <span className="stat-value lost-color">
            {voiceMailStats.notPlayed}
          </span>
        </div>
      </div>
    </div>
