...
export default function CoordinatorDashboard() {
  ...
  const [forwardRating, setForwardRating] = useState({}); // Add rating for forwarding
  ...
  const handleConvertOrForward = async (ticketId) => {
    ...
    try {
      ...
      // If converting to inquiry, use ticket's sub_section as responsible_unit_name
      if (category) {
        payload.category = category;
        const ticket = tickets.find(t => t.id === ticketId);
        if (ticket && ticket.sub_section) {
          payload.responsible_unit_name = ticket.sub_section;
        } else {
          setSnackbar({
            open: true,
            message: "Ticket does not have a sub-section assigned. Cannot convert to inquiry.",
            severity: "warning"
          });
          return;
        }
      ...
      if (unitId) {
        ...
        if (ticket && ticket.complaint_type) {
          ...
          setSnackbar({
            open: true,
            message: "Please select a rating (Minor or Major) before forwarding.",
            severity: "warning"
          });
          return;
        }
      ...
      if (response.ok) {
        ...
        setForwardRating(prev => ({ ...prev, [ticketId]: "" }));
        fetchTickets();
      } else {
              ...
              <Grid container spacing={2}>
                ...
                <Grid item xs={12} sm={6}>
                  <Typography>
                    ...
                    >
                      {selectedTicket.complaint_type || "Not Rated"}
                    </span>
                  </Typography>
                </Grid>
                <Grid item xs={12} sm={6}>
              ...
              >
                ...
                <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
                  ...
                  <Button
                    ...
                    disabled={!convertCategory[selectedTicket.id]}
                    title={!convertCategory[selectedTicket.id] ? "Please select conversion type" : ""}
                  >
                    Convert
                  </Button>
                  {convertCategory[selectedTicket.id] && (
                    <Typography variant="caption" color="info.main" sx={{ fontSize: "0.7rem" }}>
                      Will use: {selectedTicket.sub_section || "No sub-section"}
                    </Typography>
                  )}
                </Box>

                {/* Forward with Rating Section */}
                <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
                  <select
                    style={{
                      padding: "4px 8px",
                  ...
                  <select
                    ...
                    value={forwardRating[selectedTicket.id] || ""}
                    onChange={(e) =>
                      setForwardRating((prev) => ({
                        ...prev,
                        [selectedTicket.id]: e.target.value
                      }))
                    }
