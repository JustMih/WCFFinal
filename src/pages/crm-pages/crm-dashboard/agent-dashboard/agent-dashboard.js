...
export default function AgentCRM() {
  ...
  const renderTableRow = (ticket, index) => (
    <tr key={ticket.id}>
      ...
      {activeColumns.includes("Section") && (
        <td>{ticket.functionData?.parentFunction?.section?.name}</td>
      )}
      {activeColumns.includes("Sub-section") && (
        <td>{ticket.functionData?.parentFunction?.name}</td>
      )}
